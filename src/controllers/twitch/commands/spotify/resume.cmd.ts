...
export default class ResumeCMD extends WaiterCommand {
  ...
  public async exec(channel: TwitchClient, message: ChannelMessage): Promise<any> {
    ...
    try {
      await spotify.playback.resume();
      await this.bot.channel(channel).sendMessage(`Playback resumed.`, { replyTo: message });
    } catch (error) {
      this.logger.error("Error resuming playback:", error);
      await this.bot.channel(channel).sendMessage(`Failed to resume playback. Please make sure Spotify is open and try again.`, { replyTo: message });
    }
  }
}