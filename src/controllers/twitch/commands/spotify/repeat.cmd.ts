...
export default class RepeatCMD extends WaiterCommand {
  ...
  public async exec(channel: TwitchClient, message: ChannelMessage): Promise<any> {
    ...
    const mode = this.getArgs(message, "type") as "track" | "context" | "off" || "track";

    await spotify.playback.setRepeatMode(mode);
    return this.bot.channel(channel).sendMessage(`Repeat mode updated to ${mode}!`, { replyTo: message });
  }
}